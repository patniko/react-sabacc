...
export default function Player(props: PlayerProps) {
    ...
    const betButton = isBettingPhase(gamePhase) || isMatchingBetPhase(gamePhase) ?
        <button className="btn btn-dark" onClick={props.onBet}>{isBettingPhase(gamePhase) ? "Bet" : "Match bet"}</button> : null;

    const dontBetButton = isBettingPhase(gamePhase) ?
        <button className="btn btn-dark" onClick={props.onDontBet}>Don't bet</button> : null;

    const foldButton = isMatchingBetPhase(gamePhase) ?
        <button className="btn btn-dark" onClick={props.onFold}>Fold</button> : null;

    const callHandButton = canCallHand(props.gameState) ?
        <button className="btn btn-dark" onClick={props.onCallHand}>Call hand</button> : null;

    const startNextHandButton = props.gameState.gamePhase === GamePhases.HandResults ?
        <button className="btn btn-dark" onClick={props.onStartNewHand}>Start next hand</button> : null;

    const drawButton = isDrawingPhase(gamePhase) ?
        <button className="btn btn-dark" onClick={props.onDraw}>Draw</button> : null;

    const standButton = isDrawingPhase(gamePhase) ?
        <button className="btn btn-dark" onClick={props.onStand}>Stand</button> : null;
